	...
	while ($pos < $len) {
		...
		if ($charAt == '%') {
			...
			if ($charAt == 'u') {
				...
				$decodedStr .= codepointToUtf8 (hexdec ($hexVal));
				$pos += 2;
			}
		} else {
			$decodedStr .= $charAt;
        ...
        <script type="javascript">
           ...
           var step2callback = function(){
                if (processType == "createAclTemplate"){
                    ...
                    YAHOO.haloacl.callAction("haclDoesArticleExists", {articletitle:$('create_acl_general_name').value,type:protect}, function(result){
			var resulttext = result.responseText.replace(/^\s|\s+$/g, '');
                        //if (result.responseText == " true"){
                        if (resulttext == "true"){
                            step2callbackSecondCheck();
                            ...
                            YAHOO.haloacl.notification.createDialogOk("content","","$setexistingname result: '" + result.responseText + "'" + resulttext + "'",{
                                yes:function(){}
                            });
                        }
                    });
