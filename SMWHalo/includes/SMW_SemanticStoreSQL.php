	...
	protected function createPreDefinedPages($verbose) {
		...
		foreach($ssp as $key => $value) {
			...
			if (strtolower($ssp[SMW_SSP_HAS_DOMAIN_AND_RANGE_HINT]) == strtolower($t->getText())) {
				...
				$edit_flags = EDIT_FORCE_BOT;
				if ($exists) {
					$edit_flags |= EDIT_UPDATE;
				} else {
					$edit_flags |= EDIT_NEW;
				}
				$article->doEdit($text, "", $edit_flags);
		...
		foreach($scs as $key => $value) {
			...
			if (!$t->exists()) {
				$article = new Article($t);
				$article->doEdit(wfMsg('smw_predefined_cats', $t->getText()), "", EDIT_NEW | EDIT_FORCE_BOT);
				DBHelper::reportProgress("   ... Create page ".$t->getNsText().":".$t->getText()."...\n",$verbose);
			}
		}
		...
		$article = new Article($t);
		if (!$t->exists()) {
			$article->doEdit($contents, "", EDIT_NEW | EDIT_FORCE_BOT);
			DBHelper::reportProgress("   ... Create page ".$t->getNsText().":".$t->getText()."...\n",$verbose);
		} else {
